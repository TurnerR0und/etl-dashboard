...
def clean_salary_data(content: bytes) -> pd.DataFrame | None:
    ...
    # Use pd.read_excel for .xls files. We need to specify the sheet and header row.
    ...
    # For this dataset, the year is fixed (2025), so we add it as a column
    df['year'] = 2025
    df['weekly_pay'] = pd.to_numeric(df['weekly_pay'], errors='coerce')
    df.dropna(inplace=True)

    # Convert weekly pay to annual salary
    df['average_annual_salary'] = df['weekly_pay'] * 52
    
    # Map ONS region names to our house price region names
    region_mapping = {
        'East': 'East of England',
        'Yorkshire and The Humber': 'Yorkshire and The Humber',
        # This dataset is simpler, but a mapping might still be needed
    }
